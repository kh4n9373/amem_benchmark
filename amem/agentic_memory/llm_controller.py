...
class OpenAIController(BaseLLMController):
    def __init__(self, model: str = "gpt-4", api_key: Optional[str] = None, base_url: Optional[str] = None, disable_thinking: bool = False):
        try:
            ...
            # Support custom base_url for local LLMs
            if base_url:
    ...
    def get_completion(self, prompt: str, response_format: dict, temperature: float = 0.7) -> str:
        ...
        # Add disable_thinking for Qwen models if requested
        if self.disable_thinking and "qwen" in self.model.lower():
...
class LLMController:
    ...
    def __init__(self, 
        ...
        if backend == "openai":
            self.llm = OpenAIController(model, api_key, base_url, disable_thinking)
        elif backend == "ollama":
